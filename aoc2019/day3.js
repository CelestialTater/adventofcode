...
trackLine = (wire, xnum, ynum, cnum) =>{
    for(i in wire){
        switch(wire[i].substring(0,1)){
            case "R":
                var j = 0;
                while(j < wire[i].substring(1, wire[i].length)){
                    xnum++
                    cnum.push([xnum, ynum])
                    j++
                }
...
trackDistance = (wire, intersect, distance) =>{
    let xn = 0;
    let yn = 0;
    let test1 = intersect[0]
    let test2 = intersect[1]
    var ree = true
    for(var f in wire){
        while(ree){
            if(xn + yn < test1 + test2){
                ree = false
            }else{
                switch(wire[f].substring(0,1)){
                    case "R":
                        var j = 0;
                        while(j < wire[f].substring(1, wire[f].length)){
                            distance++
                            xn++
                            j++
                        }
...
console.log(trackLine(wire3, x1, y1, coords1));
console.log(trackLine(wire4, x2, y2, coords2));

for (var j in coords2) {
    duplicates[coords2[j]] = coords2[j];
}

for (var i in coords1) {
    if (typeof duplicates[coords1[i]] != 'undefined') {
        intersects.push(coords1[i])
    } 
}
console.log(intersects)
//---PART 1 SOLUTION---
// var currentLowest = 999999;
// for(i in intersects){
//     if(intersects[i][1] + intersects[i][0] < currentLowest){
//         currentLowest = intersects[i][1] + intersects[i][0]
//     }
// }
// console.log(currentLowest)

//Not working part 2 solution
let lowestDistance = 9999;
var current;
var d1;
var d2;
for(i of intersects){
    d1 = trackDistance(wire3, i, 0);
    d2 = trackDistance(wire4, i, 0);
    console.log(d1)
    console.log(d2)
    current = d1 + d2;
    console.log(current)
